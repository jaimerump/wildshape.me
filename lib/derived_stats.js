...
function computeSavingThrows(creature) {
  //return creature.ability_scores;
  ...
  return _.reduce( ABILITY_SCORES, (acc, attribute) => {
    ...
    if( creature.save_overrides && !_.isNil(creature.save_overrides[attribute]) ) {
      ...
      // How do we figure out proficiency?
      if( !_.isNil(creature.proficiency) ) {
        acc[attribute] = modifierFromAbilityScore( creature.ability_scores[attribute] ) + creature.proficiency
      } else {
        acc[attribute] = modifierFromAbilityScore( creature.ability_scores[attribute] ) + proficiencyByChallengeRating( creature );  
      }
