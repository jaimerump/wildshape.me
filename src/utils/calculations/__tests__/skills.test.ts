...
describe('getSkillAbility', () => {
  ...
  it('should throw error for unknown skill', () => {
    expect(() => getSkillAbility('InvalidSkill')).toThrow(
      'Unknown skill: InvalidSkill'
    );
  });
});
...
describe('getSkillBonus', () => {
  ...
  it('should return ability modifier only when not proficient', () => {
    ...
    expect(
      getSkillBonus('Athletics', abilityScores, proficiencyBonus, null)
    ).toBe(2);
  });
  ...
  it('should return ability modifier + PB when proficient', () => {
    ...
    expect(
      getSkillBonus('Stealth', abilityScores, proficiencyBonus, 'proficient')
    ).toBe(6);
  });
  ...
  it('should return ability modifier + 2xPB when expertise', () => {
    ...
    expect(
      getSkillBonus('Perception', abilityScores, proficiencyBonus, 'expertise')
    ).toBe(10);
  });
  ...
  it('should handle negative ability modifiers without proficiency', () => {
    ...
    expect(
      getSkillBonus('Deception', abilityScores, proficiencyBonus, null)
    ).toBe(-1);
  });
  ...
  it('should handle negative ability modifiers with proficiency', () => {
    ...
    expect(
      getSkillBonus('Deception', abilityScores, proficiencyBonus, 'proficient')
    ).toBe(2);
  });
  ...
  it('should handle different skills using different abilities', () => {
    ...
    expect(
      getSkillBonus('Athletics', abilityScores, proficiencyBonus, 'proficient')
    ).toBe(5);

    // Acrobatics (DEX 16 = +3), proficient = +6
    expect(
      getSkillBonus('Acrobatics', abilityScores, proficiencyBonus, 'proficient')
    ).toBe(6);

    // Arcana (INT 10 = +0), proficient = +3
    expect(
      getSkillBonus('Arcana', abilityScores, proficiencyBonus, 'proficient')
    ).toBe(3);
...
describe('getAllSkillBonuses', () => {
  ...
  it('should calculate all skill bonuses with no proficiencies', () => {
    ...
    const result = getAllSkillBonuses(
      abilityScores,
      proficiencyBonus,
      skillProficiencies
    );
